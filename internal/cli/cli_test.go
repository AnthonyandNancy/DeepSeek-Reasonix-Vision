...
func TestProvidersWithMissingKeysIncludesVisionModel(t *testing.T) {
	t.Setenv("DEEPSEEK_API_KEY", "set")
	...
	cfg.Agent.VisionModel = "vision-test/vision-x"

	got := providersWithMissingKeys(cfg)
	if len(got) != 1 || got[0].APIKeyEnv != "VISION_API_KEY" {
		t.Fatalf("vision model's missing key must be prompted, got %+v", got)
	}
}
