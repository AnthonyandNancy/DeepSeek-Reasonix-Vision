...
// original reference text and on-disk path so the image router can choose the
// native multimodal path, ModLens v2 evidence fallback, or an honest path-only
// degradation without re-reading the attachment.
...
// resolveInputImages resolves image references independently of the selected
// main model's capabilities. This is what lets a text-only main model hand the
// same bytes to the configured vision evidence model.
	...
	for _, r := range c.detectRefs(line) {
		...
		if err != nil {
			if isImageRefCandidate(r) {
				images = append(images, ResolvedImage{Ref: r.raw, Path: r.path, Error: "image file unreadable or unsupported"})
			}
			continue
		}
		images = append(images, ResolvedImage{Ref: r.raw, Path: r.path, DataURL: url})
	}
	return images
}
...
// inputImages remains the capability-gated helper used by legacy direct-image
// callers. Routed turns use resolveInputImages and routeImagesOnce instead.
...
// imageRefNoteFunc renders an image-attachment note. A Controller passes a
// route-aware renderer so the provider-visible context matches the actual image
// route selected later in the turn.
...
// imageRefNote explains the route that will actually handle this image. Keeping
// this note aligned with routeImagesOnce prevents a text-only main model from
// needlessly calling a second visual tool after ModLens evidence was injected.
func (c *Controller) imageRefNote(displayPath, mime string, size int64, attached bool) string {
	...
	case c.resolveVisionModelStatus().Kind == VisionModelSupported:
		if mime == "" {
			return "[image attachment available at @" + displayPath + "; the main model is text-only, so the configured vision evidence model will extract ModLens v2 evidence before the main model runs.]"
		}
		return fmt.Sprintf("[image file %s, mime=%s, %d bytes — the main model is text-only, so the configured vision evidence model will extract ModLens v2 evidence before the main model runs.]", displayPath, mime, size)
	default:
		if mime == "" {
