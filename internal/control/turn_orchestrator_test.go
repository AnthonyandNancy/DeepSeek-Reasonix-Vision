...
func TestTurnOrchestratorPromptSubmitBlockSkipsVisualEvidenceAndMainModel(t *testing.T) {
	dir := t.TempDir()
	t.Chdir(dir)
	...
	if desc.calls != 0 {
		t.Fatalf("vision evidence model called %d times despite a blocking UserPromptSubmit hook", desc.calls)
	}
	if len(runner.inputs) != 0 {
...
func TestTurnOrchestratorInjectsModLensEvidenceNotRawImages(t *testing.T) {
	dir := t.TempDir()
	t.Chdir(dir)
	writeImageRouteConfig(t, dir)
	ref, err := SaveImageDataURL("data:image/png;base64," + tinyPNG)
	...
	if desc.calls != 1 {
		t.Fatalf("vision evidence calls = %d, want exactly 1", desc.calls)
	}
	if len(runner.inputs) != 1 {
	...
	if strings.Contains(runner.inputs[0], "data:image/") || strings.Contains(runner.inputs[0], "base64,") {
		t.Fatalf("raw image leaked into text-only main model input: %q", runner.inputs[0])
	}
}
