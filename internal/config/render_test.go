...
func TestGeminiVisionProviderRoundTripsWithoutModelDefaults(t *testing.T) {
	cfg := Default()
	cfg.Providers = append(cfg.Providers, ProviderEntry{
		Name: "gemini-custom", Kind: "gemini", BaseURL: "https://generativelanguage.googleapis.com/v1beta",
		Models: []string{"custom-vision-model", "custom-text-model"}, Default: "custom-vision-model",
		VisionModels: []string{"custom-vision-model"}, APIKeyEnv: "GEMINI_API_KEY",
	})
	rendered := RenderTOMLForScope(cfg, RenderScopeUser)
	if !strings.Contains(rendered, `kind        = "gemini"`) || !strings.Contains(rendered, `vision_models = ["custom-vision-model"]`) {
		t.Fatalf("Gemini provider missing from render:\n%s", rendered)
	}
	var decoded Config
	if _, err := toml.Decode(rendered, &decoded); err != nil {
		t.Fatalf("decode Gemini config: %v", err)
	}
	entry, ok := decoded.Provider("gemini-custom")
	if !ok || entry.Kind != "gemini" || entry.DefaultModel() != "custom-vision-model" || !entry.HasVisionModel("custom-vision-model") {
		t.Fatalf("Gemini provider did not round-trip: %+v, found=%v", entry, ok)
	}
}
