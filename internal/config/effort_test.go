...
func TestGeminiEffortCapabilityUsesNativeThinkingLevels(t *testing.T) {
	e := &ProviderEntry{Kind: "gemini", BaseURL: "https://generativelanguage.googleapis.com/v1beta", Model: "custom-vision-model"}
	cap := EffortCapabilityForEntry(e)
	if !cap.Supported || cap.Default != "auto" || !containsString(cap.Levels, "low") || !containsString(cap.Levels, "high") || !containsString(cap.Levels, "disabled") {
		t.Fatalf("Gemini capability = %+v", cap)
	}
	for _, level := range []string{"low", "medium", "high", "max", "disabled"} {
		if got, err := NormalizeEffort(e, level); err != nil || got != level {
			t.Fatalf("NormalizeEffort(%q) = %q/%v", level, got, err)
		}
	}
}
