...
func TestNormalizeLegacyDesktopProviderAccessIncludesVisionModelProvider(t *testing.T) {
	c := Default()
	...
	c.Providers = append(c.Providers, ProviderEntry{
		...
		Model:        "vision-x",
		VisionModels: []string{"vision-x"},
	})
	c.Agent.VisionModel = "vision-only/vision-x"

	NormalizeLegacyDesktopProviderAccess(c)
	access := desktopProviderAccessMap(c.Desktop.ProviderAccess)
	if !access["vision-only"] {
		t.Fatalf("provider_access = %+v, want configured vision-only provider visible", c.Desktop.ProviderAccess)
	}
}
