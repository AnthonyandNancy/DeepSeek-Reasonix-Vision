...
func build(ctx context.Context, opts Options) (*BuildResult, error) {
	...
	modelSupportsImages := func(ref string) bool {
		...
		if effectiveResolver != nil {
			return config.EffectiveVision(syntheticEntryFromResolver(effectiveResolver, ref))
		}
		return false
	}
	...
	if visionModelRef != "" {
		var visionEntry *config.ProviderEntry
		if resolved, ok := cfg.ResolveModel(visionModelRef); ok {
			visionEntry = resolved
		} else if effectiveResolver != nil {
			visionEntry = syntheticEntryFromResolver(effectiveResolver, visionModelRef)
		}
		if visionEntry == nil {
			sink.Emit(event.Event{Kind: event.Notice, Level: event.LevelWarn, Text: fmt.Sprintf("vision_model %q was not found; visual fallback is disabled", visionModelRef)})
		} else if !config.EffectiveVision(visionEntry) {
			sink.Emit(event.Event{Kind: event.Notice, Level: event.LevelWarn, Text: fmt.Sprintf("vision_model %q is not marked as image-capable; visual fallback is disabled", visionModelRef)})
		} else {
			vp, vErr := resolveProvider(effectiveResolver, cfg, proxySpec, provider.Selection{Ref: visionModelRef})
			if vErr != nil {
				sink.Emit(event.Event{Kind: event.Notice, Level: event.LevelWarn, Text: "Vision evidence model could not start; visual fallback is disabled.", Detail: vErr.Error()})
			} else {
				visionDescriber = vision.NewProviderDescriber(vp, visionEntry.Price, sink)
			}
		}
	}
	...
	readOnlySkillRunner := func(sctx context.Context, sk skill.Skill, task string, runOpts skill.SubagentRunOptions) (string, error) {
		...
		runOptions.ToolImages = toolImageProcessor
		runOptions.ModelSupportsImages = modelSupportsImages(usageModelRef)
