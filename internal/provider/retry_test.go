...
func TestWithMaxRetriesZeroPinsSingleHTTPAttempt(t *testing.T) {
	calls := 0
	cl := &http.Client{Transport: rtFunc(func(r *http.Request) (*http.Response, error) {
		calls++
		return statusResp(http.StatusServiceUnavailable, nil), nil
	...
	_, err := SendWithRetry(ctx, cl, SendOptions{Provider: "vision"}, newDummyReq)
	if err == nil {
		t.Fatal("SendWithRetry error = nil, want retryable status error")
	}
	if calls != 1 {
		t.Fatalf("HTTP calls = %d, want exactly one when retry budget is zero", calls)
	}
}
