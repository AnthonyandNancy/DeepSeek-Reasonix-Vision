...
func reservedExtraBodyField(name string) bool {
	...
	case "model", "messages", "tools", "stream", "stream_options", "temperature", "max_tokens", "max_completion_tokens", "max_output_tokens", "reasoning_effort", "thinking", "response_format":
		return true
	default:
		return false
	}
...
func (c *client) openStream(ctx context.Context, targetURL string, wireReq chatRequest, tools []provider.ToolSchema) (<-chan provider.Chunk, error) {
	...
	newReq := func(ctx context.Context) (*http.Request, error) {
		...
		httpReq.Header.Set("Accept", acceptHeader(wireReq.Stream))
	...
	if wireReq.Stream || isEventStream(resp) {
		go c.streamOnce(requestCtx, resp, out)
	} else {
		go c.streamJSON(requestCtx, resp, out)
	}
	return out, nil
}
