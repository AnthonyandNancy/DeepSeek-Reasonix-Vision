...
func TestVisionClientAllowsSlowResponseHeaders(t *testing.T) {
	p, err := New(provider.Config{Name: "vision", BaseURL: "https://vision.example", Model: "vision-model", Extra: map[string]any{"vision": true}})
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	transport, ok := p.(*client).http.Transport.(*http.Transport)
	if !ok {
		t.Fatalf("transport = %T, want *http.Transport", p.(*client).http.Transport)
	}
	if transport.ResponseHeaderTimeout < 5*time.Minute {
		t.Fatalf("vision response header timeout = %s, want at least 5m", transport.ResponseHeaderTimeout)
	}
}
