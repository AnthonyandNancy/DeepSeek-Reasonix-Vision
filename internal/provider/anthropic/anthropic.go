...
func (c *client) buildRequest(_ context.Context, req provider.Request) anthRequest {
	...
	if req.ResponseFormat != nil && req.ResponseFormat.Type == "json_object" {
		r.OutputConfig = &outputConfig{Format: &outputFormat{Type: "json_schema", Schema: json.RawMessage(`{"type":"object"}`)}}
	}
	...
	if c.deepseek {
		...
		case "adaptive":
			...
			if c.effort != "" {
				...
				r.OutputConfig.Effort = c.effort
			}
