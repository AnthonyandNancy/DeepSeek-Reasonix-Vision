...
// maxRetriesKey carries a per-request retry budget override. A value of zero
// means exactly one HTTP attempt. The visual evidence bridge uses this so its
// outer three-attempt policy does not multiply provider transport retries.
type maxRetriesKey struct{}

// WithMaxRetries overrides the number of transport retries after the initial
// request. A negative value restores the package default.
...
// MaxRetriesFromContext returns a per-request override when one is present.
func MaxRetriesFromContext(ctx context.Context) (int, bool) {
	if ctx == nil {
		return 0, false
	}
