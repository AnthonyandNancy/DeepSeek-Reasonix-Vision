...
// image-capable executor. Dedicated text-only planners must not intercept it.
...
type Options struct {
	...
	// ToolImages post-processes image-bearing tool results for text-only models.
	ToolImages vision.ToolImageProcessor
	// ModelSupportsImages is the local metadata verdict for this agent model.
	ModelSupportsImages bool
}
