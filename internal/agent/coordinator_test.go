...
func TestCoordinatorDirectImageTurnBypassesDedicatedPlanner(t *testing.T) {
	planner := &mockProvider{name: "planner", streams: [][]provider.Chunk{{
	...
	if len(planner.requests) != 0 {
		t.Fatalf("dedicated planner calls = %d, want 0 on a direct-image execution turn", len(planner.requests))
	}
	if got := lastUser(exec.lastReq); got != "look at this" {
		t.Errorf("executor saw %q, want raw input", got)
	}
}

func TestCoordinatorDirectImagePlanOnlyUsesExecutorProviderWithoutTools(t *testing.T) {
	planner := &mockProvider{name: "planner", streams: [][]provider.Chunk{{
		{Type: provider.ChunkText, Text: "planner must not run"}, {Type: provider.ChunkDone},
	}}}
	exec := &mockProvider{name: "executor", streams: [][]provider.Chunk{{
		{Type: provider.ChunkText, Text: "image-aware plan"}, {Type: provider.ChunkDone},
	}}}
	policy := func(_ context.Context, _ string) PlannerDecision {
	...
	if len(planner.requests) != 0 {
		t.Fatalf("dedicated planner calls = %d, want 0", len(planner.requests))
	}
	if len(exec.requests) != 1 {
		t.Fatalf("executor provider calls = %d, want one plan-only request", len(exec.requests))
	}
	req := exec.requests[0]
	if len(req.Tools) != 0 {
		t.Fatalf("image-aware plan exposed tools: %v", req.Tools)
	}
	if len(req.Messages) == 0 {
		t.Fatal("image-aware plan request had no messages")
	}
	last := req.Messages[len(req.Messages)-1]
	if last.Role != provider.RoleUser || len(last.Images) != 1 {
		t.Fatalf("image-aware plan request = %+v, want one raw image on user message", last)
	}
}
