...
func TestSetVisionModelPersistsAndSurfacesInSettings(t *testing.T) {
	isolateDesktopUserDirs(t)
	...
	app := NewApp()
	if err := app.SetVisionModel("custom/vision-pro"); err != nil {
		t.Fatalf("SetVisionModel: %v", err)
	}
	if got := app.Settings().VisionModel; got != "custom/vision-pro" {
		t.Fatalf("Settings().VisionModel = %q, want custom/vision-pro", got)
	}
	persisted := config.LoadForEdit(config.UserConfigPath())
	if got := persisted.Agent.VisionModel; got != "custom/vision-pro" {
		t.Fatalf("persisted agent.vision_model = %q, want custom/vision-pro", got)
	}
	if err := app.SetVisionModel(""); err != nil {
		t.Fatalf("SetVisionModel(clear): %v", err)
	}
	if got := config.LoadForEdit(config.UserConfigPath()).Agent.VisionModel; got != "" {
		t.Fatalf("cleared agent.vision_model = %q, want empty", got)
	}
}

func TestSetVisionModelRejectsUnknownModel(t *testing.T) {
	isolateDesktopUserDirs(t)
	app := NewApp()
	if err := app.SetVisionModel("missing/vision"); err == nil {
		t.Fatal("SetVisionModel accepted an unknown model")
	}
}
