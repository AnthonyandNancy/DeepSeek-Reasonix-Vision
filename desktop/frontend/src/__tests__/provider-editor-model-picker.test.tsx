...
function renderGeminiProviderEditor() {
  return (
    <LocaleProvider>
      <ProviderEditor
        initial={{
...
await act(async () => {
  root.render(renderGeminiProviderEditor());
  await flushPromises();
});
ok(rootEl.textContent?.includes("Gemini GenerateContent") === true, "provider editor labels the Gemini GenerateContent protocol");
ok(rootEl.textContent?.includes("models/{model}:generateContent") === true, "Gemini provider editor previews the official GenerateContent endpoint");
...
ok(rootEl.textContent?.includes("https://api.deepseek.com/responses") === true, "Responses editor previews the Responses endpoint");
