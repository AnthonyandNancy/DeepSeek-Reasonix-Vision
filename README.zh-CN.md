...
- **ModLens v2 视觉证据回退（本分支）**：纯文本主模型可通过 `[agent].vision_model`
  自动把用户图片或工具返回图片转换成经过校验的 OCR / 布局 / 语义 / uncertainty
  证据后再推理；原生多模态模型仍走直接传图路径。详见
  [`docs/VISUAL_EVIDENCE.md`](docs/VISUAL_EVIDENCE.md) 与 [`MERGE_NOTES.md`](MERGE_NOTES.md)。
